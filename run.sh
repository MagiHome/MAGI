...
sudo docker run -d -it --privileged --name="hass" -v $(pwd)/config:/config -v /dev/snd:/dev/snd -v /etc/localtime:/etc/localtime:ro --net=host magihome/homeassistant-dev:latest 

...
sudo docker run -d -it --privileged --name="airplay" -v $(pwd)/config/airplay/shairport-sync.conf:/usr/local/etc/shairport-sync.conf --net=host magihome/airplay:latest
